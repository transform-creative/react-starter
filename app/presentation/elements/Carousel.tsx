...
>(function Carousel(
  ...
  const clonesPerSide = useInfiniteLoop
    ? Math.min(items.length, 5)
    : 0;
  const domItems = useInfiniteLoop
    ? [
        ...items.slice(-clonesPerSide),
        ...items,
        ...items.slice(0, clonesPerSide),
      ]
    : items;
   ...
   */
  const breakoutStyles: React.CSSProperties =
    fullScreen
      ? {
          width: `${width}vw`,
          position: "relative",
  ...
  useGSAP(
    () => {
      ...
      if (totalItems === 0 || !trackRef.current)
        return;
      const existingDraggable = Draggable.get(
        trackRef.current,
      );
  ...
  function getTargetIndex() {
    ...
    if (centerFocused) {
      ...
      const centerOffset =
        container.width / 2 - itemWidth / 2;
      return (
        Math.round(
          ((scrolled + centerOffset) /
            track.width) *
            domCount +
            snapOffset / 100,
        ) - cloneOffset
    ...
    return (
      Math.round(
        domCount * xPercent + snapOffset / 100,
      ) - cloneOffset
    );
  }
  ...
  function scrollToIndex(index: number) {
    ...
    if (useInfiniteLoop) {
      ...
      gsap.to(trackRef.current, {
        x: 0,
        xPercent:
          -centeredPercent(index + cloneOffset) *
          100,
        duration: speed,
        ease: "back.out",
      });
  ...
  const onMouseLeave = () => {
    ...
    if (
      !Draggable.get(trackRef.current)?.isDragging
    ) {
      autoplay &&
        carouselExtendsScreen() &&
